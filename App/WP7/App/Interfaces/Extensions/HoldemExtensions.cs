...
namespace TexasHoldemCalculator.Interfaces.Extensions
{
    public static class HoldemExtensions
    {
        ...
        public static void DisplayAsyncMessage(this PhoneApplicationPage view, string message)
        {
            view.Dispatcher.BeginInvoke(() => MessageBox.Show(message));
        }

        public static void BeginInvoke(this PhoneApplicationPage view, Action action)
        {
            view.Dispatcher.BeginInvoke(action);
        }

        public static void Try(this ISkyDriveSecurityProvider skyDrive, Action toPerform, Action<Exception> onError)
        {
            try
            {
                toPerform();
